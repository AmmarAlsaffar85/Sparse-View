...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=1, num_classes=1,
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                #d = [tensor.cpu().detach().numpy() for tensor in x_downsample]
                #shapes = [array.shape for array in d]
                #1 in case (4,4)
                #2 in case (4,4,4)
                #print(shapes)
                #print(c.shape,inx)
