...
# Preallocate pred_y_all on the GPU
pred_y_all = torch.zeros(patches_all_interp_siogram.shape[0],patches_all_interp_siogram.shape[1],patches_all_interp_siogram.shape[2],dtype=torch.float16, device='cpu')

batch_size=args.correcbatch

pred_y_all[int(xx.size(0)/batch_size)-1:,:,:]=patches_all_interp_siogram[int(xx.size(0)/batch_size)-1:,:,:]


for i in range (0,int(xx.size(0)/batch_size)): 
        
        """ Reading image """
        x=xx[i*batch_size:(i+1)*batch_size,:,:,:].to(device, dtype=torch.float32)            ## (512, 512)
           
        with torch.no_grad():
            """ Prediction and Calculating FPS """
            
            pred_y = model(x)
            pred_y_all[i*batch_size:(i+1)*batch_size,:,:]=pred_y.squeeze(1).cpu()
         
      
# Move the final result to CPU if needed
pred_y_all = pred_y_all.numpy()
...
float_array=numpy.float32(pred_y_all)
float_array.tofile(output_file)
output_file.close()

'''
fn3 = '/lhome/alsaffar/ctutil/PatchesAll.scan'
a3 = array.array('h')
pred_y_all=numpy.fromfile(open(fn3, 'rb'),dtype=numpy.float16)
print('pred_y_all',len(pred_y_all))
pred_y_all=pred_y_all.reshape(patches_all_interp_siogram.shape[0],patches_all_interp_siogram.shape[1],patches_all_interp_siogram.shape[2])
...
print(total_time)
print('The size of the patches from correction =',pred_y_all.shape)
...
for k in range (0,sinograminterpolated[:,:,:].shape[0]):
    ...
    #print('jjjj=',(k+1)*eachpatchsize,sinograminterpolated[:,:,:].shape[0])
    ...
    for i in range(num_patches_y):
      for j in range(num_patches_x):
        ...
        # Adjust patch slicing if needed
        patch = patches_[i, j]
        patch = patch[:y_end-y_start, :x_end-x_start]  # Trim patch
        
        reconstructed_image[y_start:y_end, x_start:x_end] += patch
        overlap_count[y_start:y_end, x_start:x_end] += 1
    #print(numpy.max(overlap_count),numpy.max(reconstructed_image),numpy.min(overlap_count),numpy.min(reconstructed_image))
...
plt.figure(5)
plt.title('reconstructed_image_all sinogram')
plt.imshow(reconstructed_image_all[0,:,:])
plt.figure(6)
plt.title('reconstructed_image_all sinogram')
plt.imshow(reconstructed_image_all[20,:,:])
plt.show()
